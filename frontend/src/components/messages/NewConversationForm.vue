<template>
  <div v-if="visible" class="space-y-2 border-b border-slate-200 bg-slate-50 px-4 py-3">
    <BaseInput
    ...
    >
      ...
      <span class="text-emerald-600">({{ selectedRecipient.email }})</span>
      <button
        type="button"
        class="ml-2 text-emerald-700 underline underline-offset-2 hover:text-emerald-900"
    ...
    <div
      ...
      class="max-h-48 overflow-y-auto rounded-lg border border-slate-200 bg-white"
    >
      <p v-if="searchingUsers" class="px-3 py-2 text-xs text-slate-500">Searching users...</p>
      <p v-else-if="recipientOptions.length === 0" class="px-3 py-2 text-xs text-slate-500">
        No users found.
      </p>
      <button
      ...
      >
        <p class="text-sm font-medium text-slate-900">{{ option.name }}</p>
        <p class="text-xs text-slate-500">{{ option.email }}</p>
      </button>
    </div>
    <BaseButton
      size="sm"
